...
The separate **Infer branches from addresses** setting derives an ephemeral hierarchy from address extension. With natural ordering, `2a` can be a child of `2`, while `20` is not; lexicographic ordering uses every proper literal prefix. Duplicate-address cards share one structural position while remaining separate exact cards. Roots form their own sibling axis; equal-depth cards under different parents do not.

**Show local Branch View** displays an archive-inspired deterministic local diagram beneath the active Deck card. It is always expanded and never appears on Desk cards, viewed cards, or Deck placeholders; its provisional toolbar and collapse control remain hidden pending a later design pass. The transparent tray can grow wider than its card within the Deck stage, and dragging its noninteractive space pans the Deck. The current strand is horizontal, one higher context appears above, and the active card's inferred and explicit departures appear below. Within one inferred parent, child addresses with the same leading extension delimiter and token kind form a continuation family: for example, `17,1,1 → 17,1,2` is distinct from `17,1A → 17,1a`. An explicit branch target inherits only later cards in its own family, bounded by the next explicit start there; other families remain separate inferred departures. Other visible cards use short stubs for hidden departures; selecting a stub expands its sole departure or asks which one to show, with one auxiliary row visible at a time. Same-strand omissions use a counted ellipsis marker instead; selecting one temporarily reveals that run with horizontal scrolling. Nodes show canonical addresses, retain exact-path identity for duplicate addresses, and preview targets according to **Preview links on hover**.

The six Branch View command-palette actions move backward or forward without wrapping, move to a known strand beginning, enter an address-inferred or explicit supplementary strand, and move to a higher inferred or explicit strand. Repeated supplementary and higher destinations remain separate available relations. When duplicate addresses or several semantic destinations remain possible, Slipbox asks for the exact card. If a card has several incoming explicit branches, the local view uses the first relationship in deterministic Deck order as its explicit higher context without reporting an error; the other relationships remain indexed and available as outgoing branches from their sources.

Six matching command-palette actions are available without default shortcuts. The existing inferred commands remain unchanged: **Cycle Deck anchor forward through inferred siblings** and **Cycle Deck anchor backward through inferred siblings** wrap and use `n` and `N` by default, while `-` moves to the inferred parent. Change any binding under **Settings → Slipbox Desk → Keyboard shortcuts** or Obsidian's hotkey settings.

...
import { setCardTooltip, clearCardTooltip } from "./card-tooltip.js";
import {
  renderedLinkAction,
...
import { type DeckMapCard } from "./deck-map.js";
import { DeckMapController } from "./deck-map-controller.js";
import {
...
export class DeckView extends ItemView {
  ...
  private readonly deckFrames = new DeckFrameScheduler({
    ...
    flush: (changed) => this.flushDeckMotion(changed),
  });
  private readonly bookmarkEdgeTabs = new DeckBookmarkTabs(clearCardTooltip);
  private readonly cardHeaderControllers = new Map<HTMLElement, CardHeaderButtonController>();
  ...
  private renderRefreshFrame: number | null = null;
  ...
  constructor(
    ...
    this.shortcutController = new DeckShortcutController({
      ...
      bindings: () => resolvedDeckKeybindings(this.plugin.settings),
    ...
    this.localBranchView = new LocalBranchViewController({
      ...
      placement: () => this.plugin.settings.branchViewPlacement,
      orientation: () => this.plugin.settings.deckOrientation,
      showTooltips: () => this.plugin.settings.showTooltips,
      previewLinksOnHover: () => this.plugin.settings.previewLinksOnHover,
      setIcon,
  ...
  override async onClose(): Promise<void> {
    ...
    this.spaceOffsetY = 0;
    this.renderedCards = [];
    this.deckMapController?.dispose();
    this.deckMapController = null;
    this.deckShellEl = null;
  }
  ...
  override onResize(): void {
    ...
    this.viewedCardSignature.scheduleLayout();
    this.deskRenderer.scheduleBranchLayout();
    this.deckMapController?.refreshLayout();
  ...
  private async renderDeck(focusFilingInput = true): Promise<void> {
    ...
    this.contentEl.dataset.deckOrientation = this.plugin.settings.deckOrientation;
    this.contentEl.dataset.deckStackModel = this.plugin.settings.deckStackModel;
    ...
    const shell = this.ensureDeckShell();
    this.reconcileDeckMap();
    this.applyDeckMapVisibility();
  ...
  private async refreshDeckCardWindow(): Promise<void> {
    ...
    const deckVersion = this.deckRenderVersion;
    this.rememberScrollPositions();
    const deckCards = this.deckCardsEl ?? space.createDiv({ cls: "slipbox-deck-cards" });
    this.deckCardsEl = deckCards;
    const filed = this.plugin.index.snapshot.filed;
    if (filed.length === 0) {
      this.renderEmptyDeck(deckCards);
    ...
    // New card managers already schedule their own measured layout. The fixed
    // card dimensions do not need another positioning/retry pass after Markdown.
    this.deckFrames.request(true);
  }

  private ensureDeckShell(): HTMLElement {
    const current = this.deckShellEl;
    ...
    this.deckShellEl = shell;
    this.deckMapController = new DeckMapController(shell, {
      navigate: (path) => this.jumpToPath(path),
      runAfterEditing: (reason, action) => {
        void this.runAfterInlineEditing(reason, action);
      },
      runAction: (action) => this.runAction(action),
  ...
  private reconcileDeckMap(): void {
    this.deckMapController?.setOrientation(this.plugin.settings.deckOrientation);
    this.deckMapController?.reconcile(
      this.deckMapCards(),
      this.deckViewport.anchorPath,
      this.bookmarkedPaths(),
      this.deckViewport.snapshot.renderedWindow,
    );
  }

  private applyDeckMapVisibility(): void {
    this.deckMapController?.setVisible(deckMapIsVisible(
      this.deckMapVisibility,
      this.plugin.settings.showDeckMap,
  ...
  private deckMapCards(): readonly DeckMapCard[] {
    const filed = this.plugin.index.snapshot.filed;
  ...
  private updateDeckMapBookmarks(bookmarkedPaths: Set<string>): void {
    this.deckMapController?.updateBookmarks(bookmarkedPaths);
  }
  ...
  private updateDeckMapActiveUi(): void {
    this.deckMapController?.updateActive(
      this.deckViewport.anchorPath,
      this.deckViewport.snapshot.renderedWindow,
    );
  }
  ...
  private async renderCardWindow(
    ...
    this.renderedCards = this.renderedCards.filter((element) => {
      ...
      this.cardSignatures.removeCard(element);
      const controller = this.cardHeaderControllers.get(element);
      controller?.disconnect();
      if (controller !== undefined) this.cardHeaderButtonControllers.delete(controller);
      this.cardHeaderControllers.delete(element);
      clearCardTooltip(element);
      element.remove();
      return false;
    });
    this.drawerTransition.retain(new Set(this.renderedCards.map((card) => card.dataset.path ?? "")));
    const mounted = new Map(this.renderedCards.map((card) => [card.dataset.path, card]));
    const jobs: Promise<void>[] = [];

    const focusDisplayIndex = activeIndex;

    for (const filedIndex of indices) {
      const card = filed[filedIndex];
      if (card === undefined) {
        continue;
      }

      const existing = mounted.get(card.path);
      if (existing !== undefined) {
        existing.dataset.index = String(filedIndex);
        existing.dataset.filedIndex = String(filedIndex);
        continue;
      }
      ...
      this.deckInteractivity.set(cardEl, filedIndex === activeIndex);
      cardEl.toggleClass(
        "is-card-focused",
      ...
      setCardStackOrder(
        cardEl,
        cardStackOrder(filedIndex, focusDisplayIndex, this.plugin.settings.deckStackModel),
      );
      ...
      const headerController = renderCardHeaderButtons({
      ...
      this.cardHeaderButtonControllers.add(headerController);
      this.cardHeaderControllers.set(cardEl, headerController);
        ...
        () => {
          ...
          beginThresholdPointerDrag({
            ...
            onDragMove: (moveEvent, dx, dy) => {
              if (intent === "pan") {
                this.spaceOffsetX = origin.x + dx;
                this.spaceOffsetY = origin.y + dy;
                this.applySpaceOffset();
                return;
              }
            ...
            onDrop: (upEvent) => {
              ...
              if (intent === "pan") {
                this.suppressDeckCardClickUntil = upEvent.win.performance.now() + DECK_CARD_CLICK_SUPPRESSION_MS;
  ...
  private clearCardHeaderButtonControllers(): void {
    ...
    this.cardHeaderControllers.clear();
  }
  ...
  private renderBookmarkEdgeTabs(
    ...
    geometry = this.deckGeometry(),
  ): void {
    const bookmarkIndices = [...bookmarkedPaths].flatMap((path) => {
      const index = this.plugin.index.filedIndexForPath(path);
      return index < 0 ? [] : [index];
    });
    const targets = geometry.anchorIndex < 0
      ? { before: null, after: null }
      : bookmarkEdgeTargets(bookmarkIndices, geometry);
    const filed = this.plugin.index.snapshot.filed;
    const descriptors: DeckBookmarkTab[] = [];
    for (const direction of ["before", "after"] as const) {
      const index = targets[direction];
      const card = index === null ? undefined : filed[index];
      if (card !== undefined) descriptors.push({
        direction, path: card.path, address: card.address,
        vertical: geometry.orientation === "vertical",
        showTooltips: this.plugin.settings.showTooltips,
      });
    }
    this.bookmarkEdgeTabs.reconcile(stage, descriptors, (target) => {
      const { direction, vertical, address, path, showTooltips } = target;
      const tab = stage.createEl("button", {
        cls: `slipbox-bookmark-edge-tab is-${direction}`,
        text: `${direction === "before" ? (vertical ? "▲" : "◀") : (vertical ? "▼" : "▶")} ${address}`,
        attr: { type: "button" },
      });
      setCardTooltip(tab, `Jump to bookmark ${address}`, showTooltips, { placement: "top", delay: 250 });
      tab.addEventListener("click", () => {
        void this.runAfterInlineEditing("bookmark-edge-jump", () => this.jumpToPath(path));
      });
      return tab;
    });
  }
  ...
  private attachBrowsingEvents(stage: HTMLElement): void {
    stage.addEventListener(
      "wheel",
      (event) => {
        const result = this.wheelController.route(event, {
          orientation: this.plugin.settings.deckOrientation,
          policy: this.plugin.settings.wheelOverCardBody,
          allowCardScrolling: this.plugin.settings.allowCardScrolling,
          anchor: this.renderedCards.find((card) => card.dataset.path === this.deckViewport.anchorPath) ?? null,
          editor: this.inlineEdit?.textarea ?? null,
          paneExtent: event.deltaMode === 2
            ? deckAxis(this.plugin.settings.deckOrientation).extent(stage.clientWidth, stage.clientHeight)
            : 0,
          now: this.contentEl.win.performance.now(),
        });
  ...
  private selectCardWithoutMoving(path: string): void {
    if (this.plugin.settings.deckStackModel === "drawer") {
      if (path !== this.deckViewport.anchorPath) void this.navigateToPath(path);
      return;
    }
    this.cancelViewportCentering();
    const previousAnchor = this.deckViewport.anchorPath;
  ...
  private positionCards(geometry = this.deckMotionGeometry()): boolean {
    ...
    const step = axis.extent(geometry.cardWidth, geometry.cardHeight) * geometry.spread;
    if (step <= 0) {
      return false;
    }
  ...
  private updateCardHeaderPlacement(card: HTMLElement, index: number, anchorIndex: number): void {
    const settings = this.plugin.settings;
    card.toggleClass("has-bottom-header",
      settings.fanHeadersAtBottom &&
      settings.deckOrientation === "vertical" &&
      settings.deckStackModel === "fan" &&
      index > anchorIndex,
    );
  }
  ...
  private updatePileAnchorFromDeck(geometry = this.deckGeometry()): boolean {
    ...
    const deckTop = deckTopForPileAnchor(
      ...
      activeCard === null ? deckFootprint.offsetHeight : geometry.cardHeight,
    );
    if (deckTop === null) {
      return false;
    }
    const top = `${deckTop}px`;
  ...
  private updateActiveUi(geometry = this.deckGeometry()): void {
    const activeIndex = this.plugin.index.filedIndexForPath(
      this.deckViewport.anchorPath,
    );
    if (activeIndex < 0) {
    ...
    for (const card of this.renderedCards) {
      ...
      const active = filedIndex === activeIndex;
      const changed = this.deckInteractivity.get(card) !== active;
      this.deckInteractivity.set(card, active);
      card.toggleClass("is-deck-anchor", active);
      this.updateCardHeaderPlacement(card, filedIndex, activeIndex);
      setCardStackOrder(card, cardStackOrder(filedIndex, activeIndex, this.plugin.settings.deckStackModel));
      if (changed) {
        this.cardFooters.setInteractive(card, active);
        this.cardSignatures.setInteractive(card, active);
      }
    }
    if (this.stageEl !== null) {
      this.renderBookmarkEdgeTabs(this.stageEl, this.bookmarkedPaths(), geometry);
    }
    this.updateDeckMapActiveUi();
    this.applyCardFocusClasses();
  }
  ...
  private queueRenderWindowRefresh(geometry = this.deckGeometry()): void {
    if (this.inlineEdit !== null || this.inlineEditStarting) {
      this.renderRefreshDeferred = true;
      return;
    }
    ...
    const stage = this.stageEl;
    const version = this.deckRenderVersion;
    this.renderRefreshFrame = this.contentEl.win.requestAnimationFrame(() => {
      this.renderRefreshFrame = null;
      this.renderRefreshPending = false;
      if (stage !== null && this.stageEl === stage && version === this.deckRenderVersion) {
  ...
  private cancelRenderWindowRefresh(): void {
    if (this.renderRefreshFrame !== null) this.contentEl.win.cancelAnimationFrame(this.renderRefreshFrame);
    this.renderRefreshFrame = null;
    this.renderRefreshPending = false;
    this.renderRefreshQueued = false;
  }

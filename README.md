...
**Show local Branch View** displays an archive-inspired navigation toolbar and deterministic local diagram beneath the active Deck card. It is expanded by default and never appears on Desk cards, viewed cards, or Deck placeholders. The current strand is horizontal, one higher context appears above, and the active card's inferred and explicit departures appear below. Other visible cards use short stubs for hidden departures; selecting a stub expands its sole departure or asks which one to show. Same-strand omissions use counted gap markers instead; selecting a gap temporarily reveals that run with horizontal scrolling. Nodes show canonical addresses, retain exact-path identity for duplicate addresses, and preview targets according to **Preview links on hover**.

The toolbar moves backward or forward without wrapping, moves to a known strand beginning, enters an address-inferred or explicit supplementary strand, and moves to a higher inferred or explicit strand. Repeated supplementary and higher controls represent separate available relations. When duplicate addresses or several semantic destinations remain possible, Slipbox asks for the exact card. If a card has several incoming explicit branches, the local view uses the first relationship in deterministic Deck order as its explicit higher context without reporting an error; the other relationships remain indexed and available as outgoing branches from their sources.
